...
# 1. FIX: Use 'BOT_TOKEN' environment variable name
# DO NOT hardcode the token here. Render provides the token via an environment variable.
...
if u face any problem please write us https://t.me/+p4A0p7WIvqQ0NTg9
...
def main() -> None:
    ...
    # Render provides PORT and a public URL (Render service name is the domain)
    PORT = int(os.environ.get('PORT', 8080))
    # Render service name: telegram-quiz-bot-001
    WEBHOOK_HOST = os.environ.get('WEBHOOK_HOST', 'https://telegram-quiz-bot-001.onrender.com') 

    if not BOT_TOKEN:
        logger.error("❌ BOT_TOKEN environment variable not found!")
        logger.error("Please ensure you set the BOT_TOKEN environment variable on Render.")
        return
    ...
    # --- WEBHOOK DEPLOYMENT SETUP FOR RENDER ---
    # The application must listen on the PORT specified by Render
    # and use the WEBHOOK_HOST to tell Telegram where to send updates.
    
    webhook_url = f"{WEBHOOK_HOST}"
    
    logger.info(f"Setting up Webhook on URL: {webhook_url}")
    
    application.run_webhook(
        listen="0.0.0.0",
        port=PORT,
        url_path="",  # Path component of the URL for the webhook (optional)
        webhook_url=webhook_url,
        allowed_updates=Update.ALL_TYPES
    )
    
    logger.info(f"✅ Bot is now listening for webhooks on port {PORT}!")
